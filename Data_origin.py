...
class _Data(object):
    ...
    def data_div(self):
        ...
        # np.save(os.getcwd() + '/data/X_train_origin_2.npy', X_train)
        # np.save(os.getcwd() + '/data/X_val_origin_2.npy', X_val)
        # np.save(os.getcwd() + '/data/X_test_origin_2.npy', X_test)
        ...
        # np.save(os.getcwd() + '/data/Y_train_origin_2.npy', Y_train)
        # np.save(os.getcwd() + '/data/Y_val_origin_2.npy', Y_val)
        # np.save(os.getcwd() + '/data/Y_test_origin_2.npy', Y_test)

        return X_train, X_val, X_test, Y_train, Y_val, Y_test

    def eventto0_1(self):
        event = pds.read_csv(os.getcwd() + '/data/listOfICMEs.csv', index_col=None)
        event['begin'] = pds.to_datetime(event['begin'], format="%Y/%m/%d %H:%M")
        event['end'] = pds.to_datetime(event['end'], format="%Y/%m/%d %H:%M")
    ...
    def read_csv(self, filename, index_col=0, header=0, dateFormat="%Y/%m/%d %H:%M", sep=','):
        '''
    ...
    def binary_focal_loss(self, gamma=2, alpha=0.25):
        ...
        def binary_focal_loss_fixed(y_true, y_pred):
            ...
            p_t = y_true * y_pred + (K.backend.ones_like(y_true) - y_true) * (K.backend.ones_like(y_true) - y_pred) \
                  + K.backend.epsilon()
...
class generator_conv1d(object):
    def __init__(self, mode='train', batch_size=16, windows=128, ratio=0.75):
        ...
        self.X_train = np.load(os.getcwd() + '/data/X_train_origin_2.npy')
        self.X_val = np.load(os.getcwd() + '/data/X_val_origin_2.npy')
        self.X_test = np.load(os.getcwd() + '/data/X_test_origin_2.npy')
        self.Y_train = np.load(os.getcwd() + '/data/Y_train_origin_2.npy')
        self.Y_val = np.load(os.getcwd() + '/data/Y_val_origin_2.npy')
        self.Y_test = np.load(os.getcwd() + '/data/Y_test_origin_2.npy')
